...
#            sp.show_process()
        ...
        self._radar_echo_sequence_df.to_excel("train_drop1800_30min.xlsx")
    ...
    def _datetimeSequenceCheck(self, dt):
        ...
        date_x_end = dt - timedelta(minutes=10)

        date_x = pd.date_range(date_start, end=date_x_end, freq='10T')
        print("date_x=",date_x)
        date_y_30min = pd.date_range(dt, end=date_end, freq='30T')
        print("date_pred30min=",date_y_30min)
        aa = date_x.append(date_y_30min)
        print("aa=",aa)
        # bb = date_x.extend(date_y_30min)
        # print("bb=",bb)
        # sys.exit()
        ...
        if dt == datetime.strptime('2018-08-24 18:00', "%Y-%m-%d %H:%M"):
            ...
            print("dt drop",dt)
            return False, []
        for date in aa:#daterange:
            if not date in self._radar_echo_df.index:
                return False, []
        return True, aa#daterange
...
#            data.close()
    ...
    def _testValDatetimeExtract(self):
        ...
        print("self._date_range=",self._date_range)
        print("train_date_range=",train_date_range)
        print("self._radar_echo_sequence_df.index=",self._radar_echo_sequence_df.index)
...
class generator(keras.utils.Sequence):
    def __init__(self, radar_echo_dataframe,
        ...
        self._period = period
        self._predict_period = predict_period
        print("self._predict_period",self._predict_period)
        self._predict_period = 6
        print("self._predict_period_2",self._predict_period)
