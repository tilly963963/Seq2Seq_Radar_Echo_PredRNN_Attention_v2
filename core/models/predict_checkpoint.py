...
from core.layers.InterDST_LSTMCell_checkpoint import InterDST_LSTMCell
import numpy as np
import sys
import gc
...
# memory= torch.Size([1, 64, 64, 64])
                ...
                torch.cuda.empty_cache()
                '''
                h_t[i - 1] = h_t[i - 1] + torch.zeros(1, dtype=h_t[i - 1].dtype, device=h_t[i - 1].device, requires_grad=True)
                h_t[i] = h_t[i] + torch.zeros(1, dtype=h_t[i].dtype, device=h_t[i].device, requires_grad=True)
                c_t[i] = c_t[i] + torch.zeros(1, dtype=c_t[i].dtype, device=c_t[i].device, requires_grad=True)
                c_t_history[i] = c_t_history[i] + torch.zeros(1, dtype=c_t_history[i].dtype, device=c_t_history[i].device, requires_grad=True)
                memory = memory + torch.zeros(1, dtype=memory.dtype, device=memory.device, requires_grad=True)
                '''
                # h_t[i], c_t[i], memory = checkpoint(self.cell_list[i],h_t[i - 1],h_t[i], c_t[i],c_t_history[i],memory)
                ...
                # h_t[i] = h_t[i].float()
                # c_t[i] = c_t[i].float()
                c_t_history[i] = c_t_history[i]#.half()
                # memory = memory.float()
                print("c_t_history=",self.list_shape(c_t_history))
